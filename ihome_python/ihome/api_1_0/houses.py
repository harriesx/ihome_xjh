...
def get_area_info():
    ...
    # 将对象转换为字典
    for area in area_li:
        area_dict_li.append(area.to_dict())#　在这里转换成字典key为aid,aname
...
# GET /api/v1.0/houses?sd=2017-12-01&ed=2017-12-31&aid=10&sk=new&p=1(aid地区，sk分类,p是展示的页数)
@api.route("/houses")
def get_house_list():
    """获取房屋的列表信息（搜索页面）"""
    start_date = request.args.get("sd", "")  # 用户想要的起始时间，没有的话就返回为空
    ...
    # 处理时间
    # str(p)time           str(f)time
    # p-字符串转换成时间   from时间转换成字符串
    try:
        if start_date:
    ...
    # 判断区域id，如果没有该值，则绕过这里，继续往下走
    if area_id:
        try:
            area = Area.query.get(area_id)
        except Exception as e:# 如果有异常，则抛出异常，并返回原点
            current_app.logger.error(e)
    ...
    try:
        if start_date and end_date:
            # 查询冲突的订单   sql语句：select * from order where order.begin_data<=end_data and order.end_data >= start_data
    ...
    # 查询数据库
    # 补充排序条件
    if sort_key == "booking":  # 入住做多  order_by().paginate()#返回分页的对象
    ...
    for house in house_li:
        houses.append(house.to_basic_dict())#to_basic_dict()转换为字典
    ...
    if page <= total_page:
        ...
        # 哈希类型
        try:
            # redis_store.hset(redis_key, page, resp_json)#  page, resp_json表示要保存的键值对
