...
function updateFilterDateDisplay() {
    ...
    var $filterDateTitle = $(".filter-title-bar>.filter-title").eq(0).children("span").eq(0);//.eq(0)index为０的类对象
    if (startDate) {
        var text = startDate.substr(5) + "/" + endDate.substr(5);
        $filterDateTitle.html(text);//.html(text)返回一个jquery对象
    } else {
...
function updateHouseData(action) {
    var areaId = $(".filter-area>li.active").attr("area-id");//.attr()用来设置属性
    ...
    $.get("/api/v1.0/houses", params, function(resp){//向后端发送前端数据，params参数
        house_data_querying = false;
        if ("0" == resp.errno) {//resp是后端返回的结果,后面的是function(data),所以data是后面一个函数的后端数据
            if (0 == resp.data.total_page) {//共０页
                $(".house-list").html("暂时没有符合您查询的房屋信息。");
            } else {
                total_page = resp.data.total_page;//action=renew 代表页面数据清空从新展示
                if ("renew" == action) {//重新加载，展示第１页，下一页怎么弄？下一页应该是前端－后端的page
                    cur_page = 1;
                    $(".house-list").html(template("house-list-tmpl", {houses:resp.data.houses}));
                } else {
                    cur_page = next_page;//next_page等于多少,后面加１了，有说
...
$(document).ready(function(){
    ...
    // 获取筛选条件中的城市区域信息
    $.get("/api/v1.0/areas", function(data){//data是后端传回的数据
        if ("0" == data.errno) {
